...
def get_embs(
    ...
    for i in trange(0, total_batch_length, forward_batch_size, leave=(not silent)):
        ...
        print("type: ")
        print(type(input_data_minibatch))
        print("got input data")
        torch.save(input_data_minibatch, "/u/scratch/s/schhina/temp_files/input_ids.pt")
        torch.save(pu.gen_attention_mask(minibatch), "/u/scratch/s/schhina/temp_files/atten_mask.pt") 
        torch.save(minibatch['stimulation'], "/u/scratch/s/schhina/temp_files/labels.pt")
        torch.save(original_lens, "/u/scratch/s/schhina/temp_files/lens.pt")
        print("saved")
        # with open("/u/scratch/s/schhina/temp_files/input_ids", "wb"
        with torch.no_grad():
           outputs = model(
...
def label_gene_embs(embs, downsampled_data, token_gene_dict):
    ...
    for i in range(embs.size()[0]):
        ...
        # print(list(dict_i.keys())[:min(len(dict_i.keys()), 5)])
        for k in dict_i.keys():
            gene_emb_dict[k].append(dict_i[k])
    for k in gene_emb_dict.keys():
        # if not gene_emb_dict[k]:
        #     continue
        print(len(gene_emb_dict[k]))
        print(len(gene_emb_dict[k][0]))
        print(len(gene_emb_dict[k][0][0]))
        print(torch.stack(gene_emb_dict[k]).shape)
        print(torch.mean(torch.stack(gene_emb_dict[k]), dim=0).shape)
        gene_emb_dict[k] = (
...
class EmbExtractor:
    ...
    def extract_embs(
        ...
        **Examples:**
        ...
        print(downsampled_data[0].keys())
        print(type(downsampled_data))
        torch.save(downsampled_data, "/u/scratch/s/schhina/temp_files/downsampled_data.pt")
        model = pu.load_model(
